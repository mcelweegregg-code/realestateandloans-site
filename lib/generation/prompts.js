...
SEO METADATA PLAN:
- Meta title: [55 chars max, keyword front-loaded. Do not append "| Gregg McElwee" or any name suffix, the character limit does not allow room for it. Save the name for the meta description instead.]
...
24. PRIMARY KEYWORD must be present in the body text, phrased the way a person would actually say it. Never repeat it to hit a number, and never contort a sentence to fit it in — if a natural opportunity is not there, leave it out.
...
SEO CHECKS:
- [ ] Primary keyword present in body text, phrased naturally — no instance forced, stacked, or repeated to hit a count
...
// One bounded corrective pass: fed the model's own Call 3 response plus the
// error-level lint findings, asking for surgical fixes only. The response
// must reuse the exact Call 3 output format so parseCall3Response works
// unchanged on it.
export function buildLintRevisionPrompt({ call3Response, findings }) {
  return `Your previous response is below. An automated lint pass found rule violations that must be corrected before the post can be used.

VIOLATIONS TO FIX:
${findings.map((f) => `- [${f.check}] ${f.detail}`).join('\n')}

Fix EXACTLY these violations and change NOTHING else — same content, same structure, same voice, same links, same length.
- A header containing a colon must be reworded so the colon is gone (do not just delete the colon; reword naturally).
- A narrative-hook opener ("Here's...", "This is the part that...") must be restated as a plain declarative sentence.
- An em dash must be replaced with a period, comma, or reworded sentence.
- A banned phrase must be reworded to say the same thing plainly.
- A missing internal link must be re-inserted using markdown link syntax.

Return your corrected response in EXACTLY the same format as before: the polished post, the social drafts, the |||OUTPUT_START|||...|||OUTPUT_END||| JSON package, then the CRAFT AUDIT section. Apply every fix consistently in the post body, the JSON package, and the social drafts.
