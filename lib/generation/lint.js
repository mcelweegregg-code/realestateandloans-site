...
// Keyword counting accepts up to two small connective words between tokens,
// so the natural phrasing the spec demands (rule 27) registers against
// keywords stored without them: "neutral real estate agent in a divorce"
// matches "neutral real estate agent divorce"; "selling a house during
// divorce" matches "selling house during divorce". The filler list is a
// closed set of articles and short prepositions — never arbitrary words —
// so tokens from unrelated clauses cannot bridge. Advisory measurement
// only; nothing gates on this count.
const KEYWORD_FILLERS = ['a', 'an', 'the', 'in', 'for', 'near', 'around', 'during', 'of', 'on', 'at', 'to', 'with', 'your'];

function countKeyword(text, keyword) {
  const tokens = keyword.toLowerCase().split(/\s+/)
    .map((t) => t.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const filler = `(?:(?:${KEYWORD_FILLERS.join('|')})\\s+){0,2}`;
  const pattern = new RegExp(tokens.join(`\\s+${filler}`), 'gi');
...
export function lintGenerationResult(pkg, primaryKeyword) {
  ...
  // Presence check only (Simo, Aug 2026): no numeric target — search engines
  // reward natural language, not keyword density. Warn only when the keyword
  // never appears at all.
  if (keywordCount === 0) {
    add('warn', 'keyword-count', 'primary keyword absent from body text (counting natural variants with small connective words)');
  }
  ...
  // --- links (0-2 are valid; only a claimed-but-absent link is a failure)
  for (const [key, url] of [['internal_link_a', post.internal_link_a], ['internal_link_b', post.internal_link_b]]) {
    if (url && !post.body_md.includes(url)) {
  ...
  if (social.linkedin.includes('[POST_URL]')) add('warn', 'linkedin-url', 'LinkedIn draft should not contain [POST_URL] or a URL; the link goes in comments');
  if (social.facebook.includes('[POST_URL]')) add('warn', 'facebook-url', 'Facebook draft should not contain [POST_URL] or a URL; the link goes in comments');
