...
// Sent to the editor when the publish pipeline fails in a way that needs a
// human: generation error, topics-load failure, fatal handler crash, or a
// topic stuck mid-generation. Editor only — Gregg cannot action these and
// the body carries stack traces.
export function failureEmail(subject, lines) {
  return { to: EDITOR_EMAIL, subject, text: lines.join('\n') };
}
