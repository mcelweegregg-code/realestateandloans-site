...
import { sendEmail, reminderEmail, reviewEmail } from './notify.js';
  ...
  for (const topic of topics) {
    ...
    try {
      ...
      if (toggle === 'off') {
        ...
        // Link-injection sweep, inline: Vercel Hobby has no spare function
        // or cron slot for a standalone job. The sweep is idempotent and
        // batch-scoped (posts.links_checked_at is null), so besides the
        // draft just saved it also picks up any unswept manual drafts. A
        // sweep failure is recorded but never blocks the publish job.
        if (!isMock()) {
          try {
            const { runLinkSweep } = await import('./links/sweep.js');
            entry.linkSweep = await runLinkSweep();
          } catch (err) {
            entry.linkSweep = { ok: false, error: err.message };
            summary.errors.push({ topicId: topic.id, step: 'link_sweep', error: err.message });
          }
        }
      }
    } catch (err) {
