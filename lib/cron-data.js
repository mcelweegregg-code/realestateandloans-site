...
// Overdue self-heal window: a topic whose scheduled day failed (transient
// generation error, cron outage) stays eligible this many days past its
// date. Older orphans need a deliberate reschedule in the admin panel —
// their memo and reminder context are stale.
const OVERDUE_LOOKBACK_DAYS = 7;
// Overdue eligibility is a status allowlist — each value is proof the
// reminder ladder engaged (or a memo was recorded) before the original date:
//   - 'generating' excluded: a draft likely exists awaiting review, and
//     regenerating would duplicate it. The hard-killed no-draft case is
//     surfaced by getStuckGeneratingTopics instead of retried.
//   - 'upcoming' excluded: the topic was never reminded (e.g. bulk-inserted
//     with an already-past date) — never surprise-publish those.
const OVERDUE_STATUSES = ['reminder_sent', 'reminder_sent_3d', 'reminder_sent_2d', 'reminder_sent_1d', 'recorded'];
...
export async function getTopicsScheduledFor(date) {
  ...
  // Due today: unchanged semantics (anything not already terminal).
  const { data: dueToday, error } = await supabase.from('topics').select('*')
    .eq('scheduled_date', date).not('status', 'in', '(published,auto_generated)');
  if (error) throw new Error(`topics read failed: ${error.message}`);
  // Overdue within the window. Ordered oldest-first and placed before
  // today's topic so the caller's per-run cap drains the backlog FIFO.
  const { data: overdue, error: overdueError } = await supabase.from('topics').select('*')
    .gte('scheduled_date', addDays(date, -OVERDUE_LOOKBACK_DAYS))
    .lt('scheduled_date', date)
    .in('status', OVERDUE_STATUSES)
    .order('scheduled_date', { ascending: true });
  if (overdueError) throw new Error(`topics read failed: ${overdueError.message}`);
...
// Topics stuck at 'generating' from before `today` with no posts row: a hard
// kill mid-generation leaves exactly this state. A 'generating' topic WITH a
// posts row is a draft awaiting editor review, not stuck — never returned.
// (topics has no updated_at, so "from a previous day" is the staleness test.)
export async function getStuckGeneratingTopics(today) {
  if (isMock()) return [];
  const supabase = await getSupabase();
  const { data: topics, error } = await supabase.from('topics').select('*')
    .eq('status', 'generating').lt('scheduled_date', today);
  if (error) throw new Error(`topics read failed: ${error.message}`);
  if (topics.length === 0) return [];
  const { data: posts, error: postsError } = await supabase.from('posts')
    .select('topic_id').in('topic_id', topics.map((t) => t.id));
  if (postsError) throw new Error(`posts read failed: ${postsError.message}`);
  const withDraft = new Set(posts.map((p) => p.topic_id));
  return topics.filter((t) => !withDraft.has(t.id));
}
