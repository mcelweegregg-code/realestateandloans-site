...
export async function runLinkSweep({ maxPosts = DEFAULT_MAX_POSTS, supabase = null, clients = null } = {}) {
  ...
  for (const post of posts) {
    ...
    try {
      ...
      // Observability: without these, "no claims to cite" and "every search
      // failed" both log as injected=0 and are indistinguishable after the
      // fact (Aug 2026, neutral-real-estate-agent-divorce).
      entry.claims = result.claimCount;
      entry.claimsDroppedVerbatim = result.claimsDroppedVerbatim;
