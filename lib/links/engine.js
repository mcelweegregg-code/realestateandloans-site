 ...
 *   claimCount: number, claimsDroppedVerbatim: number,
...
export async function injectExternalLinks({
  ...
  const { claims, droppedVerbatim } = await extractClaims(bodyMd, clients.sonnet);
  const skipped = [];
  ...
  return {
    bodyMd: workingMd, cap, wordCount,
    claimCount: claims.length, claimsDroppedVerbatim: droppedVerbatim,
    injected, flagged, surplus, skipped,
  };
}
